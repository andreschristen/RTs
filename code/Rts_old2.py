...
class Rts_AR:
    ...
    def PlotPostRt( self, i, ax=None, linestyle='-', color='black', **kwargs):
        ...
        ax.plot( y, pdf, linestyle=linestyle, color=color, **kwargs)
        ax.set_ylabel("Density")
        ax.set_xlabel(r'$R_{%d}$' % (i))

    def PlotRts( self, color='blue', median_color='red', x_jump=1, plot_area=[0.4,2.2], alpha=0.25, plot_obs_rts=False,
...
if __name__=='__main__':
    ...
    for i in range(20):
        ...
        tst = Rts_AR( clave, IP_dist=gamma( a=a, scale=scale), init_date=ZMs[clave][3]+timedelta(days=4), trim=trim, pred=0, n=n)
        tst.CalculateRts()
        tst.PlotPostRt( i=n, ax=ax, color='grey', alpha=0.7)
    ...
    tst.CalculateRts()
    tst.PlotPostRt( i=n, ax=ax)
    tst = Rts_AR( clave, IP_dist=gamma( a=3, scale=8/3), init_date=ZMs[clave][3]+timedelta(days=4), trim=trim, pred=0, n=n)
    tst.CalculateRts()
    tst.PlotPostRt( i=n, ax=ax, color='black')
    ### Analysis with a very different Serial interval
    PlotFrozenDist( erlang( a=30, scale=20/30 ), ax=ax1, linestyle='--', color='green')
    tst = Rts_AR( clave, init_date=ZMs[clave][3]+timedelta(days=4), trim=trim, pred=0, n=n)
    tst.CalculateRts()
    tst.PlotPostRt( i=n, ax=ax)
    tst = Rts_AR( clave, IP_dist=gamma( a=30, scale=20/30), init_date=ZMs[clave][3]+timedelta(days=4), trim=trim, pred=0, n=n)
    tst.CalculateRts()
    tst.PlotPostRt( i=n, ax=ax, linestyle='--', color='green')
    ax1.set_xlim((0,20))
